...
func (uc *UserCreate) SetIsUsed(b bool) *UserCreate {
	uc.mutation.SetIsUsed(b)
	return uc
}

// SetNillableIsUsed sets the "is_used" field if the given value is not nil.
func (uc *UserCreate) SetNillableIsUsed(b *bool) *UserCreate {
	if b != nil {
		uc.SetIsUsed(*b)
	}
	return uc
...
func (uc *UserCreate) SetStoreID(s string) *UserCreate {
	uc.mutation.SetStoreID(s)
	return uc
}
...
func (uc *UserCreate) AddMystoreIDs(ids ...uint64) *UserCreate {
	uc.mutation.AddMystoreIDs(ids...)
	return uc
}

// AddMystore adds the "mystore" edges to the Store entity.
func (uc *UserCreate) AddMystore(s ...*Store) *UserCreate {
	ids := make([]uint64, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return uc.AddMystoreIDs(ids...)
}
...
func (uc *UserCreate) check() error {
	if _, ok := uc.mutation.IsUsed(); !ok {
		return &ValidationError{Name: "is_used", err: errors.New(`ent: missing required field "User.is_used"`)}
	}
	if _, ok := uc.mutation.CreatedAt(); !ok {
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.MystoreIDs(); len(nodes) > 0 {
