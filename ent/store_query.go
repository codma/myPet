...
func (sq *StoreQuery) QueryUsers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, store.UsersTable, store.UsersPrimaryKey...),
		)
...
func (sq *StoreQuery) WithUsers(opts ...func(*UserQuery)) *StoreQuery {
	query := (&UserClient{config: sq.config}).Query()
...
func (sq *StoreQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Store, error) {
	...
	if query := sq.withUsers; query != nil {
		if err := sq.loadUsers(ctx, query, nodes,
			func(n *Store) { n.Edges.Users = []*User{} },
			func(n *Store, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StoreQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Store, init func(*Store), assign func(*Store, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(store.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(store.UsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(store.UsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(store.UsersPrimaryKey[0]))
